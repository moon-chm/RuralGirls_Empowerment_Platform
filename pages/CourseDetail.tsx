...
import { ArrowRight, Clock, BookOpen, Star, Users, Play } from "lucide-react";
...
const CourseDetail = () => {
  ...
  const handleCompleteItem = (moduleId: string, itemId: string) => {
    ...
    const itemIndex = updatedCourse.modules[moduleIndex].items.findIndex(
      (i) => i.id === itemId
    );

    updatedCourse.modules[moduleIndex].items[itemIndex].completed = true;

    const allItemsCompleted = updatedCourse.modules[moduleIndex].items.every(
      (item) => item.completed
    );
    ...
    const totalItems = updatedCourse.modules.reduce(
      (total, module) => total + module.items.length,
      0
    );
    const completedItems = updatedCourse.modules.reduce(
      (total, module) =>
        total + module.items.filter((item) => item.completed).length,
      0
    );
          ...
          <div className="max-w-4xl">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
              ...
              <div className="bg-white/10 backdrop-blur rounded-lg p-4">
                ...
                <p className="text-lg font-semibold text-white">
                  {course.rating.score} ({course.rating.count})
                </p>
              </div>
              <div className="bg-white/10 backdrop-blur rounded-lg p-4">
